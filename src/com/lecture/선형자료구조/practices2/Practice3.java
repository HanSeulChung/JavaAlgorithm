...
public class Practice3 {
    public static void solution(String[] genres, int[] plays) {
        ...
        for (int i = 0; i < genres.length; i++) {
            if (ht.containsKey(genres[i])) { // 해시 충돌상황 이용
                ArrayList<Song> cur = ht.get(genres[i]);

                int idx = -1;
                for (int j = 0; j < cur.size(); j++) {
                    if (plays[i] > cur.get(j).play || plays[i] == cur.get(j).play && i < cur.get(j).no) {
                        idx = j;
                        break;
                    }
                }
        ...
        htPlaySort.sort((x1, x2) -> x2.getValue() - x1.getValue()); //큰수로, 내림차순
