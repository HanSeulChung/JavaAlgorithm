package com.lecture.알고리즘.twopointers.practices;
...
public class Practice4 {
    public static ArrayList<ArrayList<Integer>> solution(int[] nums) {
        ...
        for( int i =0; i < nums.length - 2; i++){ // 포인터 두개 사용하기 때문에 두개 값을 뺀만큼만 순회한다.
            if( i==0 || i >0 && nums[i] != nums[i-1]){
                int p1 = i+1;
                int p2 = nums.length -1;
                int sum = 0 - nums[i];    // nums[p1] + nums[p2] + nums[i] = 0 아래의 if 문을 만족시키기 위해 sum 값을 저렇게 해놓음
